...
function openSyncDebugPanel(){
  ...
  var html='<div class="modal-overlay" onclick="if(event.target===this)closeModal()"><div class="modal" style="max-width:560px;width:min(560px,96vw)">'+
    ...
    '<div class="modal-footer" style="margin-top:10px"><button class="btn" onclick="_clearSyncLog();openSyncDebugPanel()" style="font-size:11px">🗑 مسح</button><button class="btn" onclick="closeModal();openCloudSyncPanel()">رجوع</button></div>'+
    '</div></div>';
  document.getElementById('modalContainer').innerHTML=html;
}
...
(function () {
  ...
  async function _ftOpenPhotoConflictModal(preview) {
    ...
    const rows = items.map((c) => {
      ...
      return `
        <div style="padding:10px 12px;border:1px solid var(--card-border);border-radius:12px;background:var(--card-bg);margin-bottom:10px">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:8px">
            <div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <label style="display:block;border:1px solid var(--card-border);border-radius:10px;padding:8px;background:var(--bg-secondary);cursor:pointer">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
                ...
                <input type="radio" name="${rowName}" value="local" checked onchange="_ftSetConflictChoice(${JSON.stringify(choiceKey)}, 'local')">
              </div>
            ...
            <label style="display:block;border:1px solid var(--card-border);border-radius:10px;padding:8px;background:var(--bg-secondary);cursor:pointer">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
                ...
                <input type="radio" name="${rowName}" value="cloud" onchange="_ftSetConflictChoice(${JSON.stringify(choiceKey)}, 'cloud')">
              </div>
      ...
      <div class="modal-overlay" onclick="if(event.target===this){window._ftSmartSyncState=null;closeModal();}">
        <div class="modal" style="max-width:920px;width:min(920px,96vw)">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
            <button class="btn" onclick="_ftSetAllConflictChoices('local')" style="font-size:12px">اعتماد المحلية للجميع</button>
            <button class="btn" onclick="_ftSetAllConflictChoices('cloud')" style="font-size:12px">اعتماد السحابية للجميع</button>
          </div>
          <div style="max-height:58vh;overflow-y:auto;padding-left:2px">${rows || '<div class="no-results">لا توجد تعارضات</div>'}</div>
          <div class="modal-footer" style="margin-top:10px">
            <button class="btn primary" onclick="_ftApplySmartSyncResolution()">✔ دمج المختار</button>
