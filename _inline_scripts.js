...
function _getFamilyGistFileName(){
  var id=FAMILY_DEVICE_ID||localStorage.getItem('ft_family_device_id')||'';
  if(!id){
    id=[...crypto.getRandomValues(new Uint8Array(3))].map(b=>b.toString(16).padStart(2,'0')).join('');
...
function _applyFamilyMetaIfNeeded(data){
  ...
  // Device ID
  if(meta.deviceId){
    try{localStorage.setItem('ft_family_device_id', meta.deviceId);}catch(e){}
  }
...
// ══════════════════════════════════════════════════════════
function closeModal(){document.getElementById('modalContainer').innerHTML='';tempPhoto=null;}
...
async function _doSilentFamilyUpload(){
  ...
  try{
    ...
    let data=JSON.parse(JSON.stringify({people,relPairs,nextId,myPersonId:myPersonId||null,uploadedAt:new Date().toISOString()}));
...
function _familyMatchLocalPerson(remote,incomingMap){
  ...
  // نحتاج حدًا أدنى حتى لا نطابق شخصًا خاطئًا
  if(best && bestScore>=5){
  ...
  try{
    ...
    let rowsHtml=okUpdates.map((u,idx)=>{
      ...
      let chips=[
        ...
        `🖼 ${stats.photos}`,`➕ ${u.newCnt}`,`🔄 ${u.updatedCnt}`,`⏭ ${u.unchangedCnt}`
      ...
      return `<div style="padding:10px 12px;border:1px solid var(--card-border);border-radius:10px;background:var(--card-bg);margin-bottom:10px">
        ...
        <div class="kin-stats" style="margin:0;padding:8px 0 0;border-top:1px dashed var(--card-border);background:transparent">${chips}</div>
      </div>`;
    }).join('');
...
function openFamilyUpdateDetails(idx){
  ...
  if(!u||!u.data)return;
  let incomingMap=(u.summary&&u.summary.incomingMap)||(_familyIncomingMap? _familyIncomingMap(u.data):new Map());
  let newHtml=(u.summary.newPeople||[]).map(p=>{
    let father=p.fatherId!=null?incomingMap.get(p.fatherId):null;
    let fatherName=father?_familyDisplayName(father):'—';
    return `<div class="sub-item" style="align-items:flex-start">
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;color:var(--text)">${_familyDisplayName(p)}</div>
        <div style="font-size:11px;color:var(--text2);margin-top:3px">الأب: ${fatherName}</div>
      </div>
      <div style="font-size:11px;color:var(--text2);white-space:nowrap">#${p.id}</div>
    </div>`;
  }).join('') || '<div class="no-results">لا يوجد أفراد جدد</div>';

  let updatedHtml=(u.summary.updatedPeople||[]).map(x=>{
    return `<div class="sub-item" style="align-items:flex-start">
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;color:var(--text)">${_familyDisplayName(x.remote)}</div>
        <div style="font-size:11px;color:var(--text2);margin-top:3px;line-height:1.7">${x.desc||'تغيير غير محدد'}</div>
      </div>
      <div style="font-size:11px;color:var(--text2);white-space:nowrap">#${x.remote.id}</div>
    </div>`;
  }).join('') || '<div class="no-results">لا يوجد تحديثات</div>';

  let st=u.summary.stats;
  document.getElementById('modalContainer').innerHTML=`<div class="modal-overlay" onclick="if(event.target===this)closeModal()"><div class="modal" style="max-width:760px;width:min(760px,96vw)">
    <h3>🔍 تفاصيل التحديث — ${u.devId}</h3>
    <div class="kin-stats" style="margin-bottom:12px">
      <div class="kin-stat">👨 ${st.males}</div>
      <div class="kin-stat">👩 ${st.females}</div>
      <div class="kin-stat">⚰️ ${st.deceased}</div>
      <div class="kin-stat">🖼 ${st.photos}</div>
      <div class="kin-stat">💍 ${st.marriages}</div>
      <div class="kin-stat">➕ ${u.summary.newPeople.length}</div>
      <div class="kin-stat">🔄 ${u.summary.updatedPeople.length}</div>
      <div class="kin-stat">⏭ ${u.summary.unchangedPeople.length}</div>
    </div>
    <div class="section-title">➕ الأفراد الجدد (${u.summary.newPeople.length})</div>
    <div class="person-list" style="max-height:33vh;overflow:auto">${newHtml}</div>
    <div class="section-title">🔄 الأفراد المحدَّثون (${u.summary.updatedPeople.length})</div>
    <div class="person-list" style="max-height:33vh;overflow:auto">${updatedHtml}</div>
    <div class="modal-footer">
      <button class="btn primary" onclick="confirmFamilyMerge(${idx})">➕ دمج</button>
...
function confirmFamilyMerge(idx){
  let u=window._familyUpdates&&window._familyUpdates[idx];
  if(!u||!u.data)return;
  let st=u.summary||_familySummarizeUpdate(u.data);
  document.getElementById('modalContainer').innerHTML=`<div class="modal-overlay" onclick="if(event.target===this)closeModal()"><div class="modal" style="max-width:520px">
    ...
    <div style="line-height:2;font-size:14px;color:var(--text)">
      ➕ ${st.newPeople.length} أشخاص جدد سيُضافون<br>
      🔄 ${st.updatedPeople.length} أشخاص ستُحدَّث بياناتهم<br>
      ⏭ ${st.unchangedPeople.length} أشخاص موجودون بدون تغيير
    </div>
    <div class="modal-footer">
      <button class="btn primary" onclick="_familyMergeOne(${idx})">✔ تأكيد الدمج</button>
...
async function exportFamilyCopy(){
  ...
  try{
    ...
    try{
      var mfResp=await fetch('./manifest.json',{cache:'no-store'});
      if(mfResp&&mfResp.ok) sourceManifest=await mfResp.json();
    }catch(ex){}

    var manifestBase=(sourceManifest&&typeof sourceManifest==='object')?sourceManifest:{};
    manifestBase.name='شجرة العائلة';
    manifestBase.short_name='العائلة';
    manifestBase.description='شجرة عائلية تفاعلية';
    manifestBase.start_url='./index.html';
    manifestBase.scope='./';
    manifestBase.display='standalone';
    manifestBase.orientation='any';
    manifestBase.background_color='#f3f4f6';
    manifestBase.theme_color='#3b82f6';
    manifestBase.lang='ar';
    manifestBase.dir='rtl';
    manifestBase.categories=Array.isArray(manifestBase.categories)?manifestBase.categories:['lifestyle','utilities'];

    // نُبقي أيقونات النسخة الحالية ونضمن وجود أيقونات أندرويد/المكشّفة
    var iconList=Array.isArray(manifestBase.icons)?manifestBase.icons.slice():[];
    var ensureIcon=function(src,sizes,purpose){
      if(!iconList.some(function(i){ return i&&i.src===src&&i.sizes===sizes&&i.purpose===purpose; })){
        iconList.push({src:src,sizes:sizes,type:'image/png',purpose:purpose});
      }
    };
    ensureIcon('icons/icon-72x72.png','72x72','any');
    ensureIcon('icons/icon-96x96.png','96x96','any');
    ensureIcon('icons/icon-128x128.png','128x128','any');
    ensureIcon('icons/icon-144x144.png','144x144','any');
    ensureIcon('icons/icon-152x152.png','152x152','any');
    ensureIcon('icons/icon-192x192.png','192x192','any');
    ensureIcon('icons/icon-384x384.png','384x384','any');
    ensureIcon('icons/icon-512x512.png','512x512','any');
    ensureIcon('icons/icon-maskable-192x192.png','192x192','maskable');
    ensureIcon('icons/icon-maskable-512x512.png','512x512','maskable');

    // دعم أفضل لأيقونة أندرويد: نضيف نسخة purpose="any maskable" إن لم تكن موجودة
    ensureIcon('icons/icon-192x192.png','192x192','any maskable');
    ensureIcon('icons/icon-512x512.png','512x512','any maskable');

    manifestBase.icons=iconList;
    var manifestContent=JSON.stringify(manifestBase,null,2);
...
function _updateFsaBtn(state){
  ...
  if(state==='idle'){
    ...
    if(bar){
      bar.style.display=document.body.classList.contains('mobile-mode')?'flex':'none';
...
(function(){
  ...
  fab.innerHTML=`
    ...
    <button class="mob-fab-btn mob-fab-top" onclick="window.scrollTo({top:0,behavior:'smooth'})" title="للأعلى">↑</button>
  `;
  document.body.appendChild(fab);

  // أظهر الزر عند التمرير للأسفل في وضع الجوال
  let _fabT=null;
  function _showFabBriefly(){if(window.scrollY<80){fab.classList.remove('visible');return;}fab.classList.add('visible');clearTimeout(_fabT);_fabT=setTimeout(()=>fab.classList.remove('visible'),2500);}
  window.addEventListener('scroll',function(){if(document.body.classList.contains('mobile-mode'))_showFabBriefly();},{passive:true});
})();
        ...
        <div style="padding:10px 12px;border:1px solid var(--card-border);border-radius:12px;background:var(--card-bg);margin-bottom:10px;position:relative;z-index:1;pointer-events:auto">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <label style="display:block;border:1px solid var(--card-border);border-radius:10px;padding:8px;background:var(--bg-secondary);cursor:pointer">
              ...
              <img src="${c.localPhoto}" alt="local" style="width:100%;max-height:220px;object-fit:cover;border-radius:8px;border:1px solid var(--card-border);background:#fff">
            </label>
            <label style="display:block;border:1px solid var(--card-border);border-radius:10px;padding:8px;background:var(--bg-secondary);cursor:pointer">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
              ...
              <img src="${c.cloudPhoto}" alt="cloud" style="width:100%;max-height:220px;object-fit:cover;border-radius:8px;border:1px solid var(--card-border);background:#fff">
            </label>
          </div>
        </div>`;
    }).join('');
      ...
      <div class="modal-overlay" onclick="if(event.target===this){window._ftSmartSyncState=null;closeModal();}">
        <div class="modal" style="max-width:920px;width:min(920px,96vw);position:relative;z-index:2;pointer-events:auto">
          ...
          <div style="font-size:12px;color:var(--text2);line-height:1.8;margin-bottom:12px;background:var(--bg-secondary);border:1px solid var(--card-border);border-radius:10px;padding:10px 12px">
            الصور غير المتعارضة ستُدمج تلقائياً. اختر لكل تعارض: المحلية أو السحابية، ثم اضغط دمج.
          </div>
