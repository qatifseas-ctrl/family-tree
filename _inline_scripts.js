...
<p><strong>💾 الحفظ المحلي:</strong> يتم تلقائياً داخل المتصفح. أما النقل أو النسخ الاحتياطي فباستخدام <strong>📤 تصدير JSON</strong> أو المزامنة السحابية.</p>
<p><strong>📤 تصدير JSON:</strong> نسخة احتياطية خفيفة — خياران: بدون صور (موصى به) أو مع الصور.</p>
<p><strong>📥 استيراد:</strong> يقبل JSON، ويقرأ ملفات HTML القديمة فقط للتوافق مع النسخ السابقة.</p>`},
...
<ol style="padding-right:18px;margin:0 0 10px;line-height:2">
  ...
  <li>لنقل الإعدادات للجهاز الثاني: استخدم <strong>📤 تصدير JSON</strong> على الجهاز الأول ثم استورد الملف في الجهاز الثاني</li>
</ol>
...
function openCloudSetup(){
  ...
  <div class="modal" style="max-width:500px">
    ...
    <div style="padding:10px 12px;background:#d1fae5;border:1px solid #10b981;border-radius:8px;font-size:12px;color:#065f46;margin-bottom:8px">
      ...
      لنقلها لجهاز آخر: بعد الإعداد استخدم <strong>📤 تصدير JSON</strong> أو المزامنة السحابية — لا حاجة لملف HTML.
    </div>
    <div style="padding:10px 12px;background:#ede9fe;border:1px solid #8b5cf6;border-radius:8px;font-size:12px;color:#5b21b6;margin-bottom:14px">
...
self.addEventListener('message',e=>{if(e.data==='SKIP_WAITING')self.skipWaiting();});`;
    ...
    async function _loadIconBytes(path){
    ...
    await Promise.all(iconFiles.map(async function(path){
      try{var bytes=await _loadIconBytes(path);if(bytes)iconData[path]=bytes;}
...
function _fsaShowFallbackBar(){
  ...
  if(bar){
    bar.style.display='flex';
    var hint=document.getElementById('fsaHint');
    if(hint)hint.textContent='متصفحك لا يدعم الحفظ التلقائي — استخدم "📤 تصدير JSON" بعد كل تعديل';
    var btn=document.getElementById('fsaSaveBtn');
    if(btn){btn.innerHTML='📤 تصدير JSON';btn.onclick=exportData;}
  }
}
...
window.addEventListener('beforeunload',function(e){
  ...
  e.preventDefault();
  e.returnValue='البيانات قد لا تُحفظ. استخدم التصدير قبل الإغلاق.';
});
window.addEventListener('appinstalled',function(){
